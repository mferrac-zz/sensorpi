...
df = pd.DataFrame(columns=['h26','t26',
                           ...
                           'moist',
                           'lux','lux2'])
df.loc[0] = np.hstack([read3,read4,hi2c,ti2c,moist,lightLevel,lightLevel2])
df['date'] = '{:%Y-%m-%d %H:%M:%S}'.format(dt.now())
#df.round(2).to_csv('room_log2.csv',index=False)
df.round(2).to_csv('room_log.csv',index=False,header=False,mode='a')
print(df.round(2))
